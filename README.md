...
**Ward detail** — the core of the app for a single ward, rendered as one continuous page of full-bleed, edge-to-edge bands (no boxed/shadowed cards at the section level) rather than a stack of separate cards on one background — each section owns its own full-width background color and flows straight into the next. A dark head band gives the ward's name, a breadcrumb, labeled badges ("Ward N", "Corporation: X", "Zone: Y", "Assembly: Z", plus a reservation badge where applicable), Share-on-WhatsApp/Copy-link buttons, and a single row of population/male/female/polling-booth stat tiles, with no gap below the site header. A sticky sub-nav (Overview / Candidates / Ward map / Amenities / Safety & climate / Ask your candidates / Feedback — all 7 tabs at every width, horizontally scrollable and auto-scrolling to keep the active tab visible on narrow screens) scroll-spies the rest of the page. Below it: a white Overview band (predecessor wards with % overlap, key areas); a light-neutral "coming soon" candidates band; a white interactive ward map section — the map canvas is padded/inset like the rest of the section's content, not edge-to-edge — with 13 amenity-type filter cards, an 800m walk-reach toggle, and a secondary dimmed-marker layer for nearby amenities in neighbouring wards within 1.6km of the boundary; a white Amenities band showing all 11 amenity types at once (no "show all" toggle), one uniform 3-column dashboard grid on wider screens with horizontal and vertical divider lines between cells — every row has a colored, light-tinted icon halo plus colored label/count text, the color driven either by how well it meets a population-derived benchmark (green/light-green/orange/red progress bar) or, for the handful of amenities with no fixed numeric target, by its planning category (blue/cyan badge) — no row has a colored card background, color reads only on the icon/text/progress-bar/badge; a "Did you know?" yellow band; a dark "Safety & climate" band (flood-prone/vulnerable counts + a temperature card) alongside a "BBMP Sahaaya" top-complaints panel, side by side on wider screens — both read seamlessly on the same dark background, no card boundary between them; a light "Ask your candidates" suggested-questions list (the page's own cool near-white background tone); and a closing bright-green feedback band linking to an external Microsoft Forms link (not pre-filled with ward info — Microsoft Forms only supports dynamic pre-fill via a form-owner-generated link this project doesn't have access to).
...
Colors, typography, and spacing follow Open City's brand guidelines, applied through a single palette (no light/dark theme toggle): a dark-green "forest" surface family for the header, footer, and hero/dark-card sections, light neutrals for body content (a cool near-white page background plus cream/beige surface tones for cards), and green/red/yellow accent families (plus a sage tone for the methodology teaser and one deliberate off-brand blue reserved for lake/pond markers, since the brand palette has no blue). Manrope is used for headings and stat numbers (the closest Google Fonts match to the brand's Aileron, which isn't freely hosted), PT Sans for body text, a 4pt spacing scale, and a 3-step material-style elevation system. Body-length text uses full-contrast ink (or its light-on-dark counterpart on forest surfaces), short metadata uses a muted tone, and captions/labels use the lightest hint tone — a deliberate 3-tier hierarchy, not an accident. Amenity types each get a small hand-coded, currentColor SVG icon (no icon font or library). There is no external UI/component framework — every component is hand-styled in `src/styles/`. See `.claude/skills/design-theme-protocols/SKILL.md` for the full token/component reference.
